...
def _get_data_fingerprint(path: Path) -> str:
    """Fingerprint the on-disk dataset so stale build_dataset caches are avoided."""
    manifest = path / 'manifest.json'
    if manifest.exists():
        return _sha256_file(manifest)

    # Fallback for datasets that pre-date manifest.json. This avoids hashing
    # large arrays on every run while still invalidating cache entries when the
    # data files are replaced in normal workflows.
    parts = []
    for file in sorted([path / 'info.json', *path.glob('*.npy')]):
        if file.exists():
            stat = file.stat()
    ...
    *,
    ...
    if cache:
        args = {
            ...
            'seed': seed,
            'data_fingerprint': _get_data_fingerprint(path),
        }
