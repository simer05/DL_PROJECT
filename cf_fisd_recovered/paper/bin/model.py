...
def main(
    ...
    *,
    ...
    _stamp_path = lib.env.get_project_dir() / '.cf_fisd_state.json'
    if _stamp_path.exists():
        import json as _json
        try:
            report['cf_fisd_state'] = _json.loads(_stamp_path.read_text())
        except Exception:
            pass
    ...
    if cf_fisd_cfg:
        _root_model = model.module if isinstance(model, nn.DataParallel) else model
        cf_fisd_cat_cards = list(dataset.compute_cat_cardinalities())
        if 'num_embeddings' in config['model']:
            _d_emb = int(config['model']['num_embeddings']['d_embedding'])
            cf_fisd_d_features = [_d_emb] * dataset.n_num_features + cf_fisd_cat_cards
        else:
        ...
        cf_fisd_teacher_names = tuple(
            cf_fisd_cfg.get('teacher_names', lib.cf_fisd.DEFAULT_TEACHER_NAMES)
        )
        ...
        _groups_cfg = cf_fisd_cfg.get('member_groups')
        if _groups_cfg is None:
            cf_fisd_member_groups = lib.cf_fisd.default_member_groups(
                int(config['model']['k']), cf_fisd_teacher_names
            )
        else:
            cf_fisd_member_groups = {k: list(v) for k, v in _groups_cfg.items()}
        cf_fisd_lambda = float(cf_fisd_cfg.get('lambda', 0.0))
        cf_fisd_variant = cf_fisd_cfg.get('variant', 'raw')
        cf_fisd_r1_param = _get_first_ensemble_layer(_root_model.backbone).r
        if cf_fisd_r1_param is None:
            raise RuntimeError(
                'cf_fisd requires a TabM-style model with first-layer ensemble scaling'
            )
        report['cf_fisd'] = {
    ...
    report['amp_enabled'] = bool(amp_enabled)
    report['amp_dtype'] = (
        'bfloat16' if amp_dtype is torch.bfloat16
        else 'float16' if amp_dtype is torch.float16
        else 'fp32'
    )
    report['gpu_names'] = lib.get_gpu_names() if torch.cuda.is_available() else []
    ...
    while config['n_epochs'] == -1 or step // epoch_size < config['n_epochs']:
        ...
        for batch_idx in tqdm(batches, desc=f'Epoch {step // epoch_size} Step {step}'):
            optimizer.zero_grad()
            task_loss = loss_fn(apply_model('train', batch_idx), Y_train[batch_idx])
            if cf_fisd_r1_param is not None and cf_fisd_lambda > 0.0:
                cf_fisd_penalty = lib.cf_fisd.cf_fisd_loss(
                    cf_fisd_r1_param,
                    cf_fisd_teachers,
                ...
                loss = task_loss + cf_fisd_lambda * cf_fisd_penalty
            else:
                loss = task_loss
            if not torch.isfinite(loss):
                report['failure'] = {
                    ...
                    'task_loss': float(task_loss.detach()) if torch.isfinite(task_loss) else None,
                }
                lib.dump_report(output, report)
